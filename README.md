...
firewall/
    ...
    install/
        ...
        deploy.sh          # Script automat de instalare cu PM2
    node-validator/
        package.json       # Node.js dependencies
        server.js          # HTTP server pentru validare IP
    dashboard/
        server.js          # Dashboard web pentru monitorizare
        package.json       # Dashboard dependencies
        config/            # Configuratie dashboard
            config.json
```
...
```bash
# Instalare completa (Node.js + PM2 + Validator + Dashboard)
./deploy.sh install
...
**Gestionare cu PM2**:
- Managementul serviciilor prin PM2 (process manager pentru Node.js)
- Pornire automata la boot (dupa configurare `pm2 startup`)
- Restart automat in caz de crash
- Logging integrat si monitorizare
- Dashboard web pentru monitorizare in timp real
...
# 4. Incarca regulile PF (PF este activat automat de deploy.sh)
pfctl -f /firewall/pf.conf

# 5. Porneste serviciile cu PM2
./deploy.sh start

# 6. Configureaza PM2 pentru pornire automata la boot
pm2 startup
# Urmeaza comanda afisata de PM2 (de ex: sudo env PATH=...)
pm2 save
...
bool DDosCheckResponse()
{
    ...
    // sau
    // DDosCheckResponse_Simple();  // Rapid
}
```
...
0. **Dashboard Web** (monitorizare in timp real)
```
http://your-server:3000
```
Features:
- Vizualizare tabele PF in timp real
- Statistici conexiuni si atacuri
- Gestiune IP-uri (adauga/sterge din tabele)
- Grafice si alerte
- Loguri sistem
...
pm2 logs
pm2 logs flash_ddos
pm2 logs ddos_dashboard

# Restart serviciu specific
pm2 restart flash_ddos
pm2 restart ddos_dashboard
...
### Logs
```bash
# PM2 logs (toate serviciile)
pm2 logs

# Node.js validator logs
pm2 logs flash_ddos
tail -f /firewall/log/flash_ddos.log

# Dashboard logs
pm2 logs ddos_dashboard
tail -f /firewall/log/ddos_dashboard.log
...
# Verifica status PM2
pm2 list
pm2 status flash_ddos

# Verifica logs PM2
pm2 logs flash_ddos --lines 50

# Verifica logs fisier
tail -50 /firewall/log/flash_ddos.log
...
# Testeaza manual (opreste PM2 mai intai)
pm2 stop flash_ddos
...
| Caracteristica | Veche (nginx+PHP) | Noua (Node.js + PM2) |
|----------------|-------------------|----------------------|
| Footprint RAM | ~50-100 MB | ~20-40 MB (cu dashboard) |
...
| Health check | Nu | Da |
| Logging | nginx access.log | PM2 + Fisiere dedicate |
| Dependinte | nginx, PHP-FPM | Node.js + PM2 |
| Process management | manual/rc.d | PM2 (auto-restart) |
| Monitorizare | Manual | Dashboard web + PM2 monit |
| Clustering | Nu | Da (PM2 cluster mode) |

## Suport

Pentru probleme sau intrebari:
1. Verifica statusul PM2: `pm2 list` si `pm2 logs`
2. Ruleaza `./deploy.sh status` pentru overview complet
3. Verifica log-urile:
   - PM2: `pm2 logs flash_ddos`
   - Fisiere: `/firewall/log/flash_ddos.log` si `/firewall/log/ddos_dashboard.log`
4. Verifica configuratia PF cu `pfctl -s rules -vv`
5. Acceseaza dashboard-ul web la `http://your-server:3000` pentru monitorizare vizuala
